...
#OBJECTIVE: to compare 3'UTR length, GC content, G content and C content between eIF4A-dep and antidep data
...
#Data import---
##3'UTR features
tpUTR <- read.csv(path.expand("Files/gencode_v27_pc_transcripts_tpUTRs_composition.csv"), header = T)
tpUTR %>%
...
chan_final_d %>%
  ...
  select(group, translation, GC_content, length, G_content, C_content) -> chan_tp_dep #662 transcripts

wal_final_d %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Waldron et al."),
         translation = rep("dependent"))%>%
  select(group,translation, GC_content, length, G_content, C_content) -> wal_tp_dep #420 transcripts

mod_final_d %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Modelska et al."),
         translation = rep("dependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> mod_tp_dep #84 transcripts

wolf_final_d %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Wolfe et al."),
         translation = rep("dependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> wolf_tp_dep #168 transcripts

rub_final_d %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Rubio et al."),
         translation = rep("dependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> rub_tp_dep #172 transcripts

iwa_hipp_final_d %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Iwasaki et al.\n(Hippuristanol)"),
         translation = rep("dependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> iwa_hipp_tp_dep #258 transcripts

iwa_roca_final_d %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Iwasaki et al.\n(RocA)"),
         translation = rep("dependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> iwa_roca_tp_dep #183 transcripts
...
chan_final_a %>%
  ...
  select(group, translation, GC_content, length, G_content, C_content) -> chan_tp_anti #617 transcripts

wal_final_a %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Waldron et al."),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> wal_tp_anti #31 transcripts

mod_final_a %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Modelska et al."),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> mod_tp_anti #31 transcripts

wolf_final_a %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Wolfe et al."),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> wolf_tp_anti #102 transcripts

rub_final_a %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Rubio et al."),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> rub_tp_anti #86 transcripts

iwa_hipp_final_a %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Iwasaki et al.\n(Hippuristanol)"),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> iwa_hipp_tp_anti #249 transcripts

iwa_roca_final_a %>%
  inner_join(tpUTR, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Iwasaki et al.\n(RocA)"),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> iwa_roca_tp_anti #176 transcripts
...
#Merging data---
total_data_tp <- rbind(chan_tp_dep, chan_tp_anti,
                       wal_tp_dep, wal_tp_anti,
                       mod_tp_dep, mod_tp_anti,
                       wolf_tp_dep, wolf_tp_anti,
                       rub_tp_dep, rub_tp_anti,
                       iwa_hipp_tp_dep, iwa_hipp_tp_anti,
                       iwa_roca_tp_dep, iwa_roca_tp_anti,
                       back) #Merging all data in a single data frame

chan <- rbind(chan_tp_dep, chan_tp_anti) #Merging for each study
wal <- rbind(wal_tp_dep, wal_tp_anti)
mod <- rbind(mod_tp_dep, mod_tp_anti)
wolf <- rbind(wolf_tp_dep, wolf_tp_anti)
rub <- rbind(rub_tp_dep, rub_tp_anti)
iwa_hipp <- rbind(iwa_hipp_tp_dep, iwa_hipp_tp_anti)
iwa_roc <- rbind(iwa_roca_tp_dep, iwa_roca_tp_anti)


#Removing unneccessary data---
rm(chan_final_d, chan_final_a, chan_tp_dep, chan_tp_anti,
   wal_final_d, wal_final_a, wal_tp_dep, wal_tp_anti,
   mod_final_d, mod_final_a, mod_tp_dep, mod_tp_anti,
   wolf_final_d, wolf_final_a, wolf_tp_dep, wolf_tp_anti,
   rub_final_d, rub_final_a, rub_tp_dep, rub_tp_anti,
   iwa_hipp_final_d, iwa_hipp_final_a, iwa_hipp_tp_dep, iwa_hipp_tp_anti,
   iwa_roca_final_d, iwa_roca_final_a, iwa_roca_tp_dep, iwa_roca_tp_anti,
   back, most_abundant, tpUTR)
...
#Plot---
##GC content
Split_GC <- ggplot(total_data_tp, aes(x = group, y = GC_content, fill = translation))+
  geom_boxplot()+
  ylim(c(0.0, 1.1)) +
...
##Length
Split_length <- ggplot(total_data_tp, aes(x = group, y = length, fill = translation))+
  geom_boxplot()+
...
##G content
Split_G <- ggplot(total_data_tp, aes(x = group, y = G_content, fill = translation)) +
  geom_boxplot() +
  ylim(c(0.00, 1.1)) +
...
##C content
Split_C <- ggplot(total_data_tp, aes(x = group, y = C_content, fill = translation)) +
  geom_boxplot()+
  ylim(c(0.00, 1.1)) +
