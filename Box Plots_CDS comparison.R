...
#OBJECTIVE: to compare CDS length, GC content, G content and C content between eIF4A-dep and antidep data
...
#Data import---
##CDS features
CDS <- read.csv(path.expand("Files/gencode_v27_pc_transcripts_CDSs_composition.csv"), header = T)
CDS %>%
...
chan_final_d %>%
  ...
  select(group, translation, GC_content, length, G_content, C_content) -> chan_cds_dep #662 transcripts

wal_final_d %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Waldron et al."),
         translation = rep("dependent"))%>%
  select(group,translation, GC_content, length, G_content, C_content) -> wal_cds_dep #420 transcripts

mod_final_d %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Modelska et al."),
         translation = rep("dependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> mod_cds_dep #84 transcripts

wolf_final_d %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Wolfe et al."),
         translation = rep("dependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> wolf_cds_dep #168 transcripts

rub_final_d %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Rubio et al."),
         translation = rep("dependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> rub_cds_dep #172 transcripts

iwa_hipp_final_d %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Iwasaki et al.\n(Hippuristanol)"),
         translation = rep("dependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> iwa_hipp_cds_dep #258 transcripts

iwa_roca_final_d %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Iwasaki et al.\n(RocA)"),
         translation = rep("dependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> iwa_roca_cds_dep #183 transcripts
...
chan_final_a %>%
  ...
  select(group, translation, GC_content, length, G_content, C_content) -> chan_cds_anti #617 transcripts

wal_final_a %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Waldron et al."),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> wal_cds_anti #31 transcripts

mod_final_a %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Modelska et al."),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> mod_cds_anti #31 transcripts

wolf_final_a %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Wolfe et al."),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> wolf_cds_anti #102 transcripts

rub_final_a %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Rubio et al."),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> rub_cds_anti #86 transcripts

iwa_hipp_final_a %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Iwasaki et al.\n(Hippuristanol)"),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> iwa_hipp_cds_anti #149 transcripts

iwa_roca_final_a %>%
  inner_join(CDS, by = c("transcript_v" = "transcript")) %>%
  mutate(group = rep("Iwasaki et al.\n(RocA)"),
         translation = rep("antidependent")) %>%
  select(group, translation, GC_content, length, G_content, C_content) -> iwa_roca_cds_anti #176 transcripts
...
#Merging data---
total_data_cds <- rbind(chan_cds_dep, chan_cds_anti,
                       wal_cds_dep, wal_cds_anti,
                       mod_cds_dep, mod_cds_anti,
                       wolf_cds_dep, wolf_cds_anti,
                       rub_cds_dep, rub_cds_anti,
                       iwa_hipp_cds_dep, iwa_hipp_cds_anti,
                       iwa_roca_cds_dep, iwa_roca_cds_anti,
                       back) #Merging all data in a single data frame

chan <- rbind(chan_cds_dep, chan_cds_anti) #Merging for each study
wal <- rbind(wal_cds_dep, wal_cds_anti)
mod <- rbind(mod_cds_dep, mod_cds_anti)
wolf <- rbind(wolf_cds_dep, wolf_cds_anti)
rub <- rbind(rub_cds_dep, rub_cds_anti)
iwa_hipp <- rbind(iwa_hipp_cds_dep, iwa_hipp_cds_anti)
iwa_roc <- rbind(iwa_roca_cds_dep, iwa_roca_cds_anti)


#Removing unneccessary data---
rm(chan_final_d, chan_final_a, chan_cds_dep, chan_cds_anti,
   wal_final_d, wal_final_a, wal_cds_dep, wal_cds_anti,
   mod_final_d, mod_final_a, mod_cds_dep, mod_cds_anti,
   wolf_final_d, wolf_final_a, wolf_cds_dep, wolf_cds_anti,
   rub_final_d, rub_final_a, rub_cds_dep, rub_cds_anti,
   iwa_hipp_final_d, iwa_hipp_final_a, iwa_hipp_cds_dep, iwa_hipp_cds_anti,
   iwa_roca_final_d, iwa_roca_final_a, iwa_roca_cds_dep, iwa_roca_cds_anti,
   back, most_abundant, CDS)
...
#Plot---
##GC content
Split_GC <- ggplot(total_data_cds, aes(x = group, y = GC_content, fill = translation))+
  geom_boxplot()+
  ylim(c(0.0, 1.1)) +
  ylab("CDS GC content (%)") +
...
##Length
Split_length <- ggplot(total_data_cds, aes(x = group, y = length, fill = translation))+
  geom_boxplot()+
  ...
  theme(axis.title.x = element_blank(), 
        ...
        plot.subtitle = element_text(size = 14, face = "italic", hjust = 0.5),
        panel.grid.minor = element_blank())+
  annotate("text", x = 2, y = 50, size=4, label = chan_label_l)+
  annotate("text", x = 3, y = 50, size=4, label = iwaH_label_l)+
  annotate("text", x = 4, y = 50, size=4, label = iwaR_label_l)+
  annotate("text", x = 5, y = 50, size=4, label = mod_label_l)+
  annotate("text", x = 6, y = 50, size=4, label = rub_label_l)+
  annotate("text", x = 7, y = 50, size=4, label = wal_label_l)+
  annotate("text", x = 8, y = 50, size=4, label = wolf_label_l)

##G content
Split_G <- ggplot(total_data_cds, aes(x = group, y = G_content, fill = translation)) +
  geom_boxplot() +
  ylim(c(0.00, 1.1)) +
...
##C content
Split_C <- ggplot(total_data_cds, aes(x = group, y = C_content, fill = translation)) +
  geom_boxplot()+
  ylim(c(0.00, 1.1)) +
